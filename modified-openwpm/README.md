# Modified OpenWPM
**Please follow the instructions specifed by OpenWPM below to run this code.**  
**We ran several OpenWPM instances simultaneouly in docker containers. Each container was run using the following command.**

```
mkdir -p PERSONA_NAME && docker run --net pub_net --ip IP_ASSIGNED_TO_PERSONA
--mount type=bind,source="$(pwd)"/PERSONA_NAME/,target=opt/OpenWPM/PERSONA_NAME/
-e DISPLAY=$DISPLAY -e PERSONA=PERSONA_NAME  -v /tmp/.X11-unix:/tmp/.X11-unix --shm-size=2g -it openwpm
```
